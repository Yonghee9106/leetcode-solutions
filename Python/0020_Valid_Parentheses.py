    ...
    def isValid(self, s: str) -> bool:
        ...
        for ch in s:
            if ch in dic_open:                                  # if ch is open bracket
                ...
                if len(stack) > 0 and stack[-1] == dic[ch]:     # and if the closed bracket is paired with the last bracket of the stack
                    stack.pop()                                 # pop it
                else:                                           # if not paired,
                    return False                                # it is not valid parentheses
        return len(stack) == 0                                  # if all open brackets ins tack list is paired, it is valid parentheses
